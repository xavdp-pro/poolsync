...
install -m 755 "$ROOT/deploy/poolsync-ctl.sh" "$BIN_DIR/poolsync-ctl"
install -m 755 "$ROOT/deploy/poolsync-test.sh" "$BIN_DIR/poolsync-test"
install -m 755 "$ROOT/deploy/read-image-clipboard.py" "$BIN_DIR/read-image-clipboard.py"
install -m 755 "$ROOT/deploy/write-image-clipboard.py" "$BIN_DIR/write-image-clipboard.py"
install -m 644 "$ROOT/deploy/com.xavdp.poolsync-restart.desktop" "$APP_DIR/com.xavdp.poolsync-restart.desktop"
install -m 644 "$ROOT/deploy/com.xavdp.poolsync-stop.desktop" "$APP_DIR/com.xavdp.poolsync-stop.desktop"
...
echo "==> Icônes menu XFCE + favoris whiskermenu"
bash "$ROOT/deploy/setup-poolsync-icons.sh" "$USER_NAME"
bash "$ROOT/deploy/setup-xfce-favorites.sh" "$USER_NAME"
