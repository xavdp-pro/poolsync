...
ssh "root@$HOST" "su - $USER_NAME -c 'export XDG_RUNTIME_DIR=/run/user/\$(id -u); systemctl --user daemon-reload; systemctl --user enable --now poolsync-agent.service poolsync-watchdog.timer; sleep 2; systemctl --user status poolsync-agent.service --no-pager | head -18'"
