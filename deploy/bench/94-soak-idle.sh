...
echo "   hub : $(curl -s -H "Authorization: Bearer $TOKEN" http://127.0.0.1:9470/api/status | python3 -c "import sys,json; d=json.load(sys.stdin); print(\", \".join(n[\"name\"]+(\" en ligne\" if n[\"online\"] else \" HORS LIGNE\") for n in d[\"nodes\"]))")"
' </dev/null
